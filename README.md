...
after cloning the repo. For the STE test harness that automates that
emulation workflow against the test plan, see
[`Test Tools/STE/README.md`](Test%20Tools/STE/README.md). Neither document
covers the mechanical/avionics side of the project.
...
1. Open `Test Tools\SHIELD Virtual Camera\SHIELD Virtual Camera` in Unity
   ...
   player lives (see [`Test Tools/STE/README.md`](Test%20Tools/STE/README.md)),
   not the project.
...
The scene's skybox, drones, and camera position are driven live by the STE
test harness over TCP rather than shared files, via listener components in
the scene. See [`Test Tools/STE/README.md`](Test%20Tools/STE/README.md) for
the STE test harness itself — the WinUI app and VB.NET test library that
automate this workflow (launching Unity/Python, running test scripts,
writing new ones) for the test plan.
